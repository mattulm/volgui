...
#
#
file *.dmp | grep PE | awk '{print $1}' | sed 's/:$//' >> vaddump.PE.w.txt
wc -l vaddump.PE.w.txt
echo " "
while read p; do 
	 md5sum $p >> vaddump.md5.w.txt; 
done < vaddump.PE.w.txt
echo " "
cat vaddump.md5.w.txt | sort | uniq >> vad.md5.F.txt
#
python $DSVT -k $APIK -f vad.md5.F.txt;
cp virustotal-search-*.csv $HOME/$CASE/evidence/volatility_vaddump.csv
